...
const gameLoop = async (gameId, user) => {
  ...
  getPrompts(gameId)
    ...
    .then(() => {
      ...
      const heroicSum = fetchHeroicSum(user.id);
      let totalBooty;
      if (heroicSum <= 2) totalBooty = bootyArray[0];
      if (heroicSum <= 4) totalBooty = bootyArray[1];
      if (heroicSum <= 6) totalBooty = bootyArray[2];
      const coinFlip = Math.round(Math.random());
      ...
      return inquirer.prompt([
        {
          type: 'list',
          message: chalk.blue(
            currentPrompts.prompt.replace(/{NAME}/g, user.username).replace(/{BOOTY}/g, totalBooty)
          ),
          name: 'choice',
